...
def simpleTokenize(string):
    ...
    """
    return [word for word in re.split(split_regex, string.lower(), flags=re.IGNORECASE) if word not in set([''])]
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda row : (row[0], tokenize(row[1])))
googleRecToToken = googleSmall.map(lambda row : (row[0], tokenize(row[1])))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda row: len(row[1])).reduce(lambda a, b : a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(10, lambda row : -1 * len(row[1]))
...
def idfs(corpus):
    ...
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda t1, t2: t1+t2)
...
def dotprod(a, b):
    ...
    """
    return sum(a[key] * b[key] for key in a.keys() if b.has_key(key))
...
def norm(a):
    ...
    """
    return math.sqrt(sum(math.pow(a[key], 2) for key in a.keys()))
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (google, amazon, similarityscore): (amazon+' '+google ,similarityscore))
